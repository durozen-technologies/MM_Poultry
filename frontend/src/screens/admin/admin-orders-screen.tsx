...
export function AdminOrdersScreen({ navigation }: { navigation: any }) {
 ...
 <Pressable
 accessibilityRole="button"
 className="h-10 px-3 flex-row items-center justify-center rounded-md bg-[#2E7D32] active:bg-[#1f5c22]"
 onPress={() => navigation.navigate("AdminNewOrder")}
 >
 <MaterialIcons name="add" size={20} className="text-white mr-1"/>
 ...
 <Text className="font-sans text-[#7a7f85]">No active boxes today.</Text>
  ...
  <Modal visible={isTotalOrdersModalOpen} transparent animationType="slide" onRequestClose={() => setIsTotalOrdersModalOpen(false)}>
    <View className="flex-1 justify-end">
      <Pressable className="absolute inset-0 bg-black/50" onPress={() => setIsTotalOrdersModalOpen(false)} />
      
      <View className="bg-[#f7f8fa] rounded-t-2xl max-h-[90%] flex-shrink mt-10">
        <View className="flex-row items-center justify-between p-4 bg-white rounded-t-2xl border-b border-[#e5e7eb] z-10 shadow-sm">
        ...
        <FlatList 
          ...
          renderItem={({ item: o }) => {
            ...
            return (
              <Pressable 
                className="bg-white rounded-xl p-4 border border-[#e5e7eb] active:bg-[#f7f8fa]"
                onPress={() => {
                  setIsTotalOrdersModalOpen(false);
                  navigation.navigate("OrderDetail", { order: o });
                }}
              >
...
const OrderListItem = React.memo(({
 ...
 order.status === 'FULFILLED' ? (order.is_billed ? 'text-[#15803d]' : 'text-[#2E7D32]') : 'text-[#5f6368]';
