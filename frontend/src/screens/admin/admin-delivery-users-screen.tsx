...
export function AdminDeliveryUsersScreen({ navigation }: { navigation: any }) {
 ...
 {/* 5. Password */}
 <View>
 <Text className="text-[#5f6368] text-sm font-bold font-semibold mb-1.5 ml-1">Password <Text className="text-error">*</Text></Text>
 <View className="relative flex-row items-center">
 <View className="absolute left-4 z-10">
...
const DeliveryUserCard = React.memo(({ user: u, onRemove }: { user: User; onRemove: (u: User) => void }) => {
 ...
 accessibilityRole="button"
 onPress={() => onRemove(u)} 
 className="w-10 h-10 rounded-lg bg-error/5 items-center justify-center active:bg-error/20 border border-error/10"
 >
