...
export function AdminRouteDispatchScreen({
 ...
 {msg ? (
 <Text className={msg.ok ? "text-[#2E7D32] mb-2": "text-error mb-2"}>{msg.text}</Text>
 ) : null}



 <Text className="text-base font-bold text-[#5f6368] font-semibold text-[#202124] mb-2">Driver</Text>
 ...
 <Text className="text-base font-bold text-[#5f6368] font-semibold text-[#202124] mb-2 mt-4">Farm loads (Optional)</Text>
 <View className="gap-2 mb-4">
 {loads.map((l) => (
 <View key={l.id} className="border border-[#e5e7eb] rounded-lg p-3">
 <Pressable onPress={() => toggleLoad(l.id)} className="flex-row justify-between items-center">
 <Text className="text-[#202124] font-semibold">
 {l.loaded_weight_kg} kg · {l.status}
 </Text>
 <MaterialIcons
 name={selectedLoadIds.has(l.id) ? "check-circle": "radio-button-unchecked"}
 size={22}
 className={selectedLoadIds.has(l.id) ? "text-[#2E7D32]": "text-[#5f6368]"}
 />
 </Pressable>
 {selectedLoadIds.has(l.id) ? (
 <TextInput
 className="mt-2 border border-[#e5e7eb] rounded-voltagent-sm px-3 py-2 text-[#202124]"
 ...
 {loads.length === 0 ? (
 <Text className="text-[#5f6368] italic">No open loads</Text>
 ) : null}
 </View>

 <Text className="text-base font-bold text-[#5f6368] font-semibold text-[#202124] mb-2">Selected</Text>
 ...
 />
 {selectedKg > 0 ? (
 <Text className="text-sm text-[#5f6368] mb-4">{selectedKg.toFixed(1)} kg total</Text>
 ) : null}

 <Text className="text-base font-bold text-[#5f6368] font-semibold text-[#202124] mb-2">Retailers</Text>
 ...
 selectedOrders.has(o.order_id) ? "border-[#2E7D32] bg-[#2E7D32]/10/10": "border-[#e5e7eb]"
 }`}
 >
 <View className="flex-row justify-between items-center mb-2">
 <Text className="text-[#202124] font-semibold">{o.shop_name ?? o.retailer_id}</Text>
 <MaterialIcons
 name={selectedOrders.has(o.order_id) ? "check-circle": "radio-button-unchecked"}
 size={22}
 className={selectedOrders.has(o.order_id) ? "text-[#2E7D32]": "text-[#5f6368]"}
 />
 </View>
 <View className="bg-[#f7f8fa]/30 rounded-lg p-2 border border-[#e5e7eb]">
 {o.items.map((it) => (
 <View
 key={it.item_id}
 className="flex-row items-center justify-between py-1 border-b border-surface-variant/30 last:border-b-0"
 >
 <Text className="text-sm font-bold text-[#5f6368] text-[#202124] font-semibold flex-1 pr-2">
 {it.item_name ?? "Item"}
 </Text>
 <View className="flex-row items-center gap-1">
 <Text className="text-[#202124]">{it.total_boxes ?? 0} Box • </Text>
 <TextInput 
 className="border border-[#e5e7eb] rounded px-2 py-0 text-[#202124] min-w-[50px] text-center"
 ...
 keyboardType="decimal-pad"
 />
 <Text className="text-[#202124]"> KG</Text>
 </View>
 </View>
 ))}
 {o.items.length === 0 ? (
 <Text className="text-[#5f6368] italic text-sm">No items listed</Text>
 ) : null}
 </View>
 </Pressable>
 ))}
 {bucket.orders.length === 0 ? (
 <Text className="text-[#5f6368]">No eligible orders for this route</Text>
 ) : null}
 </View>
