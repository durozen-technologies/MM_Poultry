...
def test_delivery_stop_not_found(client: TestClient, mock_delivery_auth: None) -> None:
    nid = str(uuid.uuid4())
    # 2. Test weigh
    res = client.post(f"/api/v1/delivery/stops/{nid}/weigh", json={"items": [{"item_id": "00000000-0000-0000-0000-000000000999", "weight_kg": 10, "delivered_boxes": 1}]})
    assert res.status_code == 404
...
def test_dispatch_without_farm_load(client: TestClient, mock_admin_auth: None) -> None:
    """Single-order dispatch modal payload: driver + order only, no farm allocations."""
    farm_resp = client.post("/api/v1/admin/farms", json={"name": "No Load Farm"})
    load_resp = client.post(
        "/api/v1/admin/farm-loads",
        json={"farm_id": farm_resp.json()["id"], "loaded_weight_kg": 1000},
    )
    load_id = load_resp.json()["id"]

    ret_resp = client.post("/api/v1/admin/retailers", json={"name": "No Load Retailer"})
    ...
    confirm_resp = client.post(
        ...
        json={"expected_delivery_date": order_resp.json().get("order_date", "08/09/2026")},
    )
    assert confirm_resp.status_code == 200

    run_resp = client.post(
        "/api/v1/admin/delivery-runs",
        json={
            "order_ids": [order_id],
            "driver_user_id": str(TEST_DELIVERY_USER_ID),
            "driver_name": "Test Driver",
        },
    )
    assert run_resp.status_code == 200, run_resp.text
    assert run_resp.json()["stops"][0]["daily_order_id"] == order_id

    got = client.get(f"/api/v1/admin/orders/today").json()
    statuses = {o["id"]: o["status"] for o in got["items"]}
    assert statuses[order_id] == "DISPATCHED"
...
def test_delivery_weigh_and_bill(client: TestClient, mock_admin_auth: None) -> None:
    ...
    err_override = client.post(f"/api/v1/delivery/stops/{stop_id}/weigh", json={
        "items": [{"item_id": "00000000-0000-0000-0000-000000000999", "weight_kg": 103.0, "delivered_boxes": 2}],
