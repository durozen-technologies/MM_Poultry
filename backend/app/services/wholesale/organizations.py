...
async def create_organization(db: AsyncSession, payload: OrganizationCreate) -> OrganizationOut:
    ...
    if existing:
        if not existing.is_active:
            ...
            await db.flush()
            return OrganizationOut.model_validate(existing, from_attributes=True)
        msg = (
    ...
    try:
        ...
        await db.rollback()
        # race: another request created same slug concurrently
        existing_race = await db.scalar(select(Organization).where(Organization.slug == slug))
        if existing_race:
            raise HTTPException(
...
async def update_delivery_user(
    ...
    try:
        ...
        user = await db.scalar(
            select(User).where(User.id == user_id, User.role == UserRole.DELIVERY)
        )
        if not user:
